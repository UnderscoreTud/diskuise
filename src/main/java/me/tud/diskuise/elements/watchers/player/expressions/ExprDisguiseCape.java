...
@Description("Set or get if the cape of a player disguise is enabled")
@Examples("set cape of disguise {dis} to true")
@Since("0.2-beta2")
@RequiredPlugins({"LibsDisguises"})
public class ExprDisguiseCape extends SimpleExpression<Boolean> {

    static {
        Skript.registerExpression(ExprDisguiseCape.class, Boolean.class, ExpressionType.PROPERTY,
                "[the] cape [(value|option|state)] of [dis(k|g)uise] %disguise%",
                "[dis(k|g)uise] %disguise%'s cape [(value|option|state)]");
    }
